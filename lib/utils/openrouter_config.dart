...
class OpenRouterConfig {
  ...
  static const Map<String, String> systemMessageTemplates = {
    ...
    'conversational': 'You are a friendly, conversational AI assistant. Engage in natural dialogue and be personable.',
    'professional': 'You are a professional AI assistant. Provide formal, business-appropriate responses with technical accuracy.',
    'educational': 'You are an educational AI assistant. Explain concepts clearly, provide examples, and encourage learning.',
  };
  ...
  static OpenRouterRequest createStandardRequest({
    ...
    final parameters = parameterPresets[preset] ?? parameterPresets['balanced']!;
  ...
  static OpenRouterRequest createConversationRequest({
    ...
    if (messages.length > maxHistoryLength) {
      ...
      if (nonSystemMessages.length > maxHistoryLength - systemMessages.length) {
        messages.addAll(nonSystemMessages.skip(nonSystemMessages.length - (maxHistoryLength - systemMessages.length)));
      } else {
  ...
  static List<String> get freeModels => popularModels.where((model) => model.contains(':free')).toList();
  ...
  static String? validateRequest(OpenRouterRequest request) {
    ...
    if (request.parameters.maxTokens != null && request.parameters.maxTokens! > 8000) {
